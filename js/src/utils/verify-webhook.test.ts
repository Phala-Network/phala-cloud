...
describe("verifyWebhookSignature", () => {
	it("returns true for valid signature", () => {
		...
		expect(
			verifyWebhookSignature({ secret: TEST_SECRET, timestamp: ts, body: TEST_BODY, signature: sig }),
		).toBe(true);
	});
	...
	it("returns false for tampered body", () => {
		...
		const sig = computeSignature(TEST_SECRET, ts, TEST_BODY);
		expect(
			verifyWebhookSignature({ secret: TEST_SECRET, timestamp: ts, body: "tampered", signature: sig }),
		).toBe(false);
	});
